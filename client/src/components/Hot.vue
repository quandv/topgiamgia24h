<template>
  <div id="hot-posts" class="row" v-if="posts.length > 0">
    <div class="col-md-12">
      <h3><i class="fa fa-star-o"></i> Tin khuyến mại HOT</h3>
    </div>
...
export default {
  ...
  watch: {
    ...
    'page' () {
      ...
      this.$router.push('/khuyen-mai-hot/' + this.oid + '/' + this.s + '/' + this.page)
    }
  },
  methods: {
    async getPosts () {
      const response = await PostsService.fetchPostsHot(this.oid, this.s, this.page)
