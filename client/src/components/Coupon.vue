<template>
  <div id="coupon-posts" class="row" v-if="posts.length > 0">
    <div class="col-md-12">
      <h3><i class="fa fa-star-o"></i> Mã giảm giá</h3>
    </div>
    <template v-for="(post, index) in posts">
      <div class="col-md-4" v-bind:key="post.id+'-abc'">
        <div class="post">
          ...
          <div class="post-body">
            <div class="post-meta">
              <router-link v-bind:to="'/khuyen-mai-hot/' + post.offer_id" class="post-category cat-2">{{ post.offer_id }}</router-link>
...
export default {
  ...
  watch: {
    ...
    'page' () {
      ...
      this.$router.push('/ma-giam-gia/' + this.oid + '/' + this.s + '/' + this.page)
    }
  },
  methods: {
    async getPosts () {
      const response = await PostsService.fetchPostsCoupon(this.oid, this.s, this.page)
    ...
    showCoupon (coupon) {
      ...
      elCopy.value = coupon
      document.getElementById('coupon-posts').appendChild(elCopy)
      elCopy.select()
      document.execCommand('copy')
      document.getElementById('coupon-posts').removeChild(elCopy)

      alert('Mã giảm giá của bạn(đã copy): ' + coupon)
    }
  }
}
</script>
